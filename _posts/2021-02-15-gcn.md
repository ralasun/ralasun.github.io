...
$$\hat{f}(\xi) = <f, e^{2\pi ix\xi}>\int_{\mathbf{R}^d} f(x)e^{2\pi ix\xi} \,dx$$ 

주파수 성분은 $e^{2\pi ix\xi}$ 이며, 여기에 laplace operator를 적용하면 아래와 같습니다.

$$Lf = -\triangle(e^{2\pi ix\xi}) = -\frac{\partial^2}{\partial x^2}e^{2\pi ix\xi} = (2\pi \xi)^2 e^{2\pi ix\xi} = \lambda f$$
...
$$\mathbf {L} = \mathbf {U^{\intercal}\Lambda U}$$

이를 이용한 graph fourier transform과 inverse graph fourier transform은 아래와 같습니다.

$$\mathcal {F}(\mathbf x) = \mathbf {U^{\intercal}x}$$
